...
Version 2.34.1:
- new: Source Code: ORK now contains the full C# source code (full version only).
- new: Menu Settings: 'Use Accept Timeout' settings available. Optionally use a (global) timeout for allowing accepting of GUI content. This is used for all types of accepting, i.e. 'Accept Key' and 'Accept Click' on inputs.
- new: Cursor Settings: Default Cursor: 'Reset to System Default' setting available. Reset to the system default cursor when no default cursor is found.
- new: Is ORK Camera Component: 'Is ORK Camera' component available. Sets the 'Camera' defined in the component as ORK's camera. E.g. use this when spawning the camera with the player, while having another camera in the scene.
- fix: Active Time Battles: Fixed a potential issue where combatants dying at the start of their turn can halt the battle under certain conditions (using 'Pause On Choosing' option).
- fix: Music: Fixed an issue where looping didn't work correctly when playing a stored music.
- fix: Menu Screens: Fixed an issue where 'Merge Sub Types' didn't work with defined types.
- fix: Player Controls: Top Down 2D: Fixed an issue where moving diagonally allowed faster movement.
- fix: Battle Grids: Generation: Fixed an issue with separate block/empty cell raycasts hitting more than they should in 2D environments.
- fix: Weapons, Armor: Durability: Fixe an issue where 'None' outworn action didn't keep the bonuses and abilities of the equipment accessible.
- fix: Battle Grids, Battle Range Templates: Fixed issues with rotating grid masks in 2D grids.
- fix: Event System: Check Transform: Fixed an issue where updating data from (very) old events caused an issue.
- fix: HUDs: Combatant, Turn Order, Latest Turn: Shortcut: Fixed an issue where displaying reuse times didn't update the shortcuts until the reuse time/turn was reduced for the first time.
- fix: HUDs: Tooltips: Fixed an issue where showing the use costs of an ability always used the player group leader for calculations.
- fix: Shops: Space: Fixed an issue where the occupied space in a shop's inventory wasn't calculated correctly.
- fix: Save/Load Menus: Fixed an issue where menu screens could be called while displaying the save/load game menus.
